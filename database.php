...
// データ取得関数（SELECT文の実行）
//引数$columにnull入れた時全データ表示
function getData($pdo, $table, $colum, $key)
{
    try {
        if ($colum == null || $key == null) {
            $stmt = $pdo->query("SELECT * FROM $table");
        } else {
            $stmt = $pdo->query("SELECT * FROM $table where $colum=$key");
        }
